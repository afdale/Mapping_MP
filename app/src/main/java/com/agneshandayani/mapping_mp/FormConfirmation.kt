...
class FormConfirmation : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnmc!!.setOnClickListener {
            val intent = Intent(this@FormConfirmation, BarcodeScannerActivity::class.java)
    ...
    inner class Douploadreport(var activity: FormUpdate) : AsyncTask<String, String, String>() {
        ...
        override fun onPostExecute(s: String) {
            dialog.dismiss()
            Toast.makeText(this@FormConfirmation, "" + z, Toast.LENGTH_LONG).show()

            if (isSuccess) {
                FirebaseDatabase.getInstance().getReference().child("mapping").child("manpower")
                    .child("ABSENSIK2").removeValue()
